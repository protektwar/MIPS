...
update_player_pos:
	...
	lw $t4,  0($gp) # $t4 now contains the color of that pixel
...
#  a ) check for input
#    li $s7, 0xffff0000
#    lw $t0, ($s7) # $t0=0 when no input available, $t0=1 when there is an input available
...
#		lw $a0, 4($s7)
	...
	beq $a0, 'w', up # 119=ascii code for w
	beq $a0, 'a', left # 97=ascii code for a
	beq $a0, 's', down #115=ascii code for s
	beq $a0, 'd', right # 110=ascii code for d
	
	beq $a0, 'x', exit # 120=ascii code for x
